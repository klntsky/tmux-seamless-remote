...
Then press `prefix` + <kbd>I</kbd> to install it. TPM normally binds this to:
...
- requirements: tmux with pane geometry formats, Bash, awk, OpenSSH, and Linux `/proc` for
...
- The local process must be able to open a separate batch-mode SSH connection
  to the same destination. Existing `ssh-agent` and `~/.ssh/config` settings
  are honored. Passphrase-protected keys work when already unlocked in the
  agent; password and passphrase prompts are never opened by a key binding.
- The exact server socket and session are addressed for every operation. The
  ...
  prove that a compatible peer exists. Worst case is that geometry-based navigation
  stops working.
